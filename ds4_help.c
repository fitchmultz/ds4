...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "DS4_GLM_PREFILL_BATCH_LIVE=1", "GLM experimental: for short Metal prompts, prefill in a scratch batch ctx, copy verified state into live ctx, and fall back to sequential prefill on failure.");
