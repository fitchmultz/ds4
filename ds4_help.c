...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "DS4_GLM_VERIFY_BATCH_TIME=1", "GLM diagnostic: time the opt-in full F32 NextN verifier batch helper by alloc/embed/attention/FFN/head; 0/off/false/no disable.");
    ...
    opt(fp, c, "DS4_GLM_PREFILL_BATCH_LIVE=1", "GLM experimental: for short Metal prompts, prefill in a scratch batch ctx, copy verified state into live ctx, and fall back to sequential prefill on failure; DS4_GLM_FAST=1 enables its fast-MoE bridge.");
