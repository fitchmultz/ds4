...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "DS4_GLM_PREFILL_BATCH_CHECK=1", "GLM diagnostic: after normal short Metal prefill, run the layer-major batch helper in a scratch ctx and report final-logit top/max-diff; does not affect output.");
