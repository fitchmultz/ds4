...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--glm-nextn", "GLM-5.2 only (opt-in, default off, Metal target only): enable the correctness-first NextN/MTP speculative greedy scaffold for --glm-raw/--glm-chat. Current drafter is CPU-side and may be slower; output stays byte-for-byte greedy-identical because every emitted token is the verified target argmax and drafts never touch the target KV cache. CPU target ignores it with a diagnostic.");
