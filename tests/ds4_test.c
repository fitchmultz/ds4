...
static void test_glm_spec_metal_target_synth(void) {
#ifdef DS4_NO_GPU
    fprintf(stderr, "  glm-spec-metal-target-synth: SKIP (built without GPU/Metal)\n");
    return;
#else
    if (!ds4_gpu_init()) {
        fprintf(stderr, "  glm-spec-metal-target-synth: SKIP (no Metal device)\n");
        return;
    }
    const int n_steps = 8;
    int full = 0, partial = 0, miss = 0;
    int rc = ds4_glm_spec_metal_target_synth(n_steps, &full, &partial, &miss);
    TEST_ASSERT(rc == 0);
    TEST_ASSERT(full == 1);
    TEST_ASSERT(partial == 1);
    TEST_ASSERT(miss == 1);
...
static void test_glm_metal_components(void) {
    ...
    const uint32_t batch_n = 2;
    float *xb = malloc((size_t)batch_n * H * sizeof(float));
    float *cqa_b = malloc((size_t)batch_n * ql * sizeof(float));
    float *mqa_b = malloc((size_t)batch_n * ql * sizeof(float));
    memcpy(xb, x, (size_t)H * sizeof(float));
    for (uint32_t i = 0; i < H; i++) xb[(size_t)H + i] = x[i] * 0.5f + 0.01f * (float)((int)i - 3);
    glm_matmul_f32(cqa_b, WqA, xb, ql, H, batch_n);
    TEST_ASSERT(ds4_gpu_glm_matmul_f32(WqA, xb, mqa_b, ql, H, batch_n));
    glm_metal_cmp("matmul q_a batch2 (WqA@X)", mqa_b, cqa_b, (size_t)batch_n * ql, tol_abs, tol_rel);
    float *cqa_bn = malloc((size_t)batch_n * ql * sizeof(float));
    float *mqa_bn = malloc((size_t)batch_n * ql * sizeof(float));
    glm_rmsnorm_batch_f32(cqa_bn, cqa_b, ones, ql, batch_n, shape.rms_eps);
    TEST_ASSERT(ds4_gpu_glm_rmsnorm_batch_f32(cqa_b, ones, mqa_bn, ql, batch_n, shape.rms_eps));
    glm_metal_cmp("rmsnorm q_a batch2", mqa_bn, cqa_bn, (size_t)batch_n * ql, tol_abs, tol_rel);
