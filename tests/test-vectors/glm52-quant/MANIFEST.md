...
`dequantize_row_*` output (one F32 per line), generated with `/tmp/gen_oracle`
linking `libggml-base.dylib`.
...
These cover every quant type the GLM-5.2 UD-IQ2_M CPU forward dequants: Q8_0
(attention projections), IQ2_XXS (most routed-expert gate/up), IQ2_S (blk.8
gate/up), IQ4_XS (routed-expert down), and the K-quants. Q2_K/Q3_K appear only
in the deferred blk.78 NextN block and are excluded. blk.8 is the ONLY IQ2_S
layer; blk.7 stands in for the IQ2_XXS layers.
