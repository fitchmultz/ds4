...
- Next meaningful bar-movers: GLM-specific persistent/batched graph (especially
  batched prefill; the small GLM F32 batch matmul/residual-add/RMSNorm/RoPE/attention Metal
  primitives plus layer-major MLA, dense-FFN, dense residual-block, MoE-router, F32 routed-MoE, and routed-FFN residual batch composites are now tested), NextN/MTP speculative decode,
