...
- `--glm-nextn` + `--glm-nextn-draft N` is the opt-in, default-off,
  ...
  scratch verifier seam with batched LM head/top-1, but target layer rows are
  still produced by single-token target steps, so the speedup still needs full
  batched target-layer verification. `DS4_GLM_NEXTN_TRACE_OUT=<path>` writes a per-round CSV
